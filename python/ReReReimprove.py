...
def log_move(shared_data, base_timestamp, current_record_folder):
    """
    レスポンスの処理、モーターの動作、応答の記録、ファイルの移動を行う関数
    """
    try:
        response_data = shared_data["gemini_response"]  # 辞書として取得
        ...
        movement_list = [] #ここに処理したmovementを追加していく
        ...
        for movement in motor_movements:
            ...
            movement_list.append(movement) # movement をリストに追加
